...
source=Path(__file__).resolve().parents[1]/'custom_components/maverick_music_flow/radio_directory.py'
...
class RadioArtworkTests(TestCase):
    def test_public_station_logo_uses_local_proxy(self):
        runtime=SimpleNamespace(register_artwork_source=lambda source:'/api/maverick_music_flow/artwork/item/token')
        ...
        self.assertEqual(stations[0]['homeii_artwork_url'],'/api/maverick_music_flow/artwork/item/token')
